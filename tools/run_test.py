...
def run_test(
    ...
    exe_path = BIN_PATH / "emxbach" / "emxbach"
    ...
    if not conv_out_path.exists() or not sub_out_path.exists():
        print(
            f"{color_print.RED}At least one X-BACH output is missing. The program did not run correctly."
        )
        return False
    ...
    (
        ...
        conv_wrong_nans,
        conv_max_coords,
    ) = diff_fits(base_test_path / f"{conv_name}_{testAgainst}.fits", conv_out_path)
    (
        sub_max_abs_err,
        sub_mean_abs_err,
        sub_max_rel_err,
        ...
        sub_wrong_nans,
        sub_max_coords,
    ) = diff_fits(base_test_path / f"{sub_name}_{testAgainst}.fits", sub_out_path)
    print(
    ...
    return (
        ...
        and sub_max_abs_err < max_abs_error[1]
        and sub_max_rel_err < max_rel_error[1]
        and conv_wrong_nans == 0
        and sub_wrong_nans == 0,
    )
...
def print_help():
    ...
    print(
        f"{color_print.YELLOW}--release: Runs the program in release mode. If not specified, the debug build is used."
    )
    print(
        ...
        case "hotpants":
            exe_args.extend(
                [
                    ...
                    "-outim",
                    str(out_path / f"{sub_name}.fits"),
                    "-oci",
                    str(out_path / f"{conv_name}.fits"),
                ]
            )
...
def generate(testAgainst, external_path, i):
    ...
    run(
        testAgainst, template_name, science_name, conv_name, sub_name, in_path, out_path
    )
