...
struct SubStampMp {
  ...
  bool operator<(const SubStampMp& other) const { return val < other.val; }
  bool operator>(const SubStampMp& other) const { return val > other.val; }
...
struct StampMp : public Stamp {
  ...
  StampMp() {};
  StampMp(std::pair<cl_long, cl_long> stampCoords,
          std::pair<cl_long, cl_long> stampSize, std::pair<cl_long, cl_long> c,
          const std::vector<SubStampMp>& subStamps,
...
class ImageMask {
 public:
  ...
  ImageMask(const std::pair<int, int>& axis)
      : axis(axis), dataMask(axis.first * axis.second, NONE) {}
  ...
  void unmask(int index, masks mask) { dataMask[index] &= ~mask; }
