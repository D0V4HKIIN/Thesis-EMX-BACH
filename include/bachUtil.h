...
/* SSS */
void createStamps(const Image& img, std::vector<Stamp>& stamps, const int w,
                  const int h, const Arguments& args);
double checkSStamp(const SubStamp& sstamp, const Image& image, ImageMask& mask,
                   const Stamp&, const ImageMask::masks badMask,
                   const bool isTemplate, const Arguments& args);
cl_int findSStamps(Stamp& stamp, const Image& image, ImageMask& mask,
                   const int index, const bool isTemplate,
                   const Arguments& args);
void identifySStamps(std::vector<Stamp>& templStamps, const Image& templImage,
...
void convStamp(Stamp& s, const Image& img, const Kernel& k, const int n,
               const int odd, const Arguments& args);
void cutSStamp(SubStamp& ss, const Image& img, const ImageMask& mask,
               const Arguments& args);
int fillStamp(Stamp& s, const Image& tImg, const Image& sImg,
              const ImageMask& mask, const Kernel& k, const Arguments& args);

/* CD && KSC */
double testFit(std::vector<Stamp>& stamps, const Image& tImg, const Image& sImg,
...
double getBackground(const int x, const int y,
                     const std::vector<double>& kernSol,
...
bool checkFitSolution(const Kernel& k, std::vector<Stamp>& stamps,
                      const Image& tImg, const Image& sImg, ImageMask& mask,
                      const Arguments& args);
